...
består av ARM-baserade MD407-datorer.

Larmsystemet ökar översikten och säkerheten av användarens lokaler genom att 
erbjuda konfiguerbar larmning av dörrar, samt rörelse- samt vibrationsdetektering.
...
Detta uppnås genom att erbjuda mindre delsystem som kan kopplas ihop till ett större 
...
För att koppla upp en dörr till systemet så krävs det två anslutningar till en av mikrodatorns
...
och avläsas på rätt sätt. Sedan startas en System Timer (SysTick) som fungerar som klocka. 
...
i den ordningen som de initierades. Detta görs för att testa om det lokala larmen fungerar hos dörrarna. 
Slutligen tänds lampan som är kopplad till pinne 2 på port B som signalerar att systemet är redo för drift.
\newpage

\begin{figure}
Systemet loopar nu kontinuerligt igenom alla dörrar enligt följande flöde: \\ \\
\centering
\includegraphics[scale=0.5]{Flow-Chart.png}¨
\caption{Dörrenhetens flödesdiagram}
\end{figure}

Dörrenhetens flöde bygger på polling. Samtliga dörrar har i sina datarepresentationer ett antal kontrollbitar,
i flödesschemat så är dessa kontrollbitar representerade som 'beslut', dvs att 1 eller 0 representerar om man 
ska ta vägen \textit{'Ja'} eller \textit{'Nej'}.

När dörrenheten larmar lokalt så tänds LED-lampan på dörren.

När dörrenheten larmar centralt så skickas ett meddelande via CAN till systemets centralenhet.

Väntetiderna för lokala- och centrala larm kan konfigueras från centralenheten. Larmtiderna sätts med 10 sekundersintervall, och den maximala väntetiden är ca 40 minuter $(256*10$ sekunder).

Dörrenheten kan köras i lokaldrift men bör för bästa effekt köras uppkopplad mot centralenheten.\\
------ \\
Dörrappendix? Diskussion kanske.

Skriva i diskussionen om varför dynamiskt?

Skriva om detta är bra därför att? Ska detta ligga i diskussionen kanske?
