...
Projektets syfte är att utveckla ett larmsystem som ska vara ett finansiellt dynamiskt alternativ för företag
och organisationer som har behov av skalbar och anpassad larmning beroende på nuvarande nödvändighet och finansiell \\ kapabilitet.
...
central för samtliga \todo[]{centrala?} larm som utlöses i systemet. Om ett centralt larm utlöses kan det endast ackkrediteras och nollställas via centralenheten.
...
På centralenheten utvecklades först USART, samt ett antal debugfunktioner som kunde användas av alla enheter. Implementeringen av CAN-kommunikation på centralenheten styrde mycket av \todo[]{påverkade i hög grad?} hur CAN implementerades på de andra enheterna, då centralenheten är den som tar emot och skickar flest meddelanden.

Den stora majoriten av mjukvaruutvecklingen och testerna har utförts på hårdvaran som beskrivs i rapporten, men en del av utvecklingen har skett med hjälp av simulerad hårdvara.
...
\begin{itemize}
    ...
    \item "Väntetid"-funktionalitet. (Delay-funktioner)\todo[]{Ändra hela raden till "Fördröjning"?}
...
\label{verifikation}\todo[inline]{Ändra tempus?}
För att utvärdera huruvida larmsystemet uppnår de mål och krav som formulerats i målet \todo[]{Referens till kapitlet?} används tester.
...
Systemet itererar nu över alla inkopplade sensorer enligt figur \ref{fig:MotionFlow}. \clearpage
...
Trig- och echo-pinnen från HC-SR04 samt en LED-lampan kopplas resepktive till  pin-$(3n)$, -$(3n+1)$ och -$(3n+2)$.
...
En pinne för digital-ouput från SW-18010P och en LED-lampa kopplas till pin-$(2n$ respektive pin-$(2n+1)$.
