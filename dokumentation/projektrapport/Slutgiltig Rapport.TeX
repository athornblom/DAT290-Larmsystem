...
\begin{description}
    \item[Startläge] I detta läge accepterar centralenheten sammankoppling med periferienheter genom att lyssna på deras ID-begäransmeddelanden och därefter tilldela dem ID:n.
    
    Då centralenheten är i detta läge kan periferienheter ansluta sig till den genom att skicka en ID-begäran via CAN, varpå centralenheten skickar tillbaka ett ID.
...
\todo[inline]{Hur mycket av följande stycke behövs egentligen?} 
...
Vibrationssensorn har endast central larm som skickas via CAN.
 ...
 För att säkerställa att funktionalitet ovan fungerar utfördes följande tester:
 \begin{enumerate}
	\item Test av portar sam lokalt och centralt larm för rörelseenheten.\ref{test:PortMotion}
 ...
 \todo[inline]{försöker så lite frön}
 Larmsystemet som helhet fungerar enligt uppgiftsbeskrivning. Det består utav
 en dörrenhet, en rörelse enhet och en centralenhet. De olika enheterna kommunicerar
 via CAN. Både dörrenheten och rörelseenheten kan skicka centrala larm till centralenheten
 efter en tid som man kan ställa in via centralenheten. Ifall en enhet försvinner ifrån
 nätverket märks detta och användaren varnas. Enligt tester kan vi också 
 med "confidence" säga att även med mycket brus på can bussen går larmen 
 fram till centralenheten. Ifall en enhet larmar centralt men inte ackkrediteras skickar
 den ett nytt larm för att säkerhetställa att meddelanden kommer fram.
