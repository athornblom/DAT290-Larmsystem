...
Larmsystemets syfte är att vara ett finansiellt dynamiskt alternativ för 
företag och organisationer som har behov av anpassad larmning beroende på 
rådande behov. Detta uppnås genom att erbjuda mindre delsystem som kan kopplas 
ihop till ett större centraliserat system.

Systemet ökar översikten och säkerheten av användarens lokaler genom \newline att 
implementera konfiguerbar larmning av dörrar, samt rörelse- och vibrationsdetektering.
...
\end{itemize}
Samtliga enheter kräver en MD407-mikrodator för individuell drift. För att 
kopplas samman fristående enheter till ett centraliserat system krävs en CAN-buss.

Dörrenheten hanterar detektering- och larmande av dörrar. Vid systemuppstart 
 dörrenheten antalet dörrar kopplade på enkortsdatorn. Datarepresentationer av
  ...
  Dörrarna har också förmågan att skicka centrala larm via en CAN-buss till 
  centralenheten. Tider innan larmen utlöstes kan konfigueras från centralenheten. 
  Från centralenheten har man även förmågan att avlarma dörrar, vilket hindrar 
  samtliga larm på dörren från att utlöstas.

Röreleseenheten består av en rörelse- och en vibrationssensor. Om endera sensor 
utlöstes skickas ett centralt larm via en CAN-buss till centralenheten. Känsligheten 
...
centrum för samtliga larm som utlöstes i systemet. Om ett centralt larm utlöstes så 
kan detta endast ackkrediteras och nollstälals från centralenheten.
...
en för varje periferienhet, då parallellt arbete ansågs effektivare.

\textbf{Centralenheten}: Gruppen består av Josef och Filip. Målet är att kunna styra alla de andra 
periferienheterna från centralenheten samt implementera CAN-bussen.
...
Eftersom samtliga portar har 16 tillgängliga pinnar innebär detta att systemet maximalt 
kan stötta 32 dörrar per utsedd dörr-mikrodator.

Port B är inte avsedd för dörranslutningar, utan används främst för CAN-\\kommunikationen 
med centralenheten. En LED-lampa kan även kopplas till \\ pinne 2 på port B. Denna lampan 
är till för att vid uppstart signalera när systemets initiering är färdig.

Vid uppstart initieras först portarna och pinnarna på mikrodatorn så att de kan addresseras 
och avläsas på rätt sätt. Därefter startas en System Timer (SysTick) vilken fungerar som klocka. 
...
hittade. Efter samtliga datarepresentationer är skapade kommer systemet att tända dörrarnas lampor 
i den ordningen som de initierades. Detta görs för att testa om det lokala larmen fungerar hos dörrarna. Efter att samtliga lampor är tända så släcker systemet dem igen.
Slutligen tänds lampan som är kopplad till pinne 2 på port B som signalerar att systemet är redo för drift.
\newpage
...
Dörrenhetens flöde bygger på polling. Samtliga dörrar har i sina datarepresentationer ett antal kontrollbitar,
i flödesschemat i figur 3 så är dessa kontrollbitar representerade som 'beslut' som ska representera om en kontrollbit är satt till 1 (för \texit{ja}) eller 0 (för \textit{nej}). Dessa kontrollbitar sätts till 1 eller 0 beroende på externa händelser.
...
Att en dörr är öppen innebär att kretsen som går via strömbrytaren på dörren inte längre är sluten. Om kretsen inte är sluten, och dörren inte är avlarmad, tyder det på att någonting externt har hänt som systemet bör tas hänsyn till. Hur lång tid systemet väntar innan det larmar lokalt sätts i form av 10-sekunders intervall, och konfigueras från centralenheten. Väntetiden kan vara allt från 0 sekunder till ca 40 minuter (Maxtiden är $2^8 *10$ sekunder).

När dörrenheten larmar centralt så skickas ett meddelande via CAN till systemets centralenhet. Meddelande kommer skickas med jämna intervall till dess att en centralenheten skickar tillbaka en ackkreditering om att larmet har mottagits. Väntetiden konfigueras från centralenheten, precis som med det lokala larmet.

Dörrenheten kan köras utan uppkoppling till centralenheten, men då går man miste om det centrala larmet, samt förmågan att konfiguera enskilda dörrar.\\ \\
